...
public class TestBackgroundLoop : MonoBehaviour
{
	...
	private void Update()
	{
        var background = levels[0];
        var buildings = levels[1];
		float offset = moveSpeed * Time.deltaTime;
		buildings.transform.position = new Vector3(buildings.transform.position.x + offset, buildings.transform.position.y, buildings.transform.position.z);
        buildings.transform.position = new Vector3(buildings.transform.position.x + offset * 2, buildings.transform.position.y, buildings.transform.position.z);
    }
}
